...
def run():
    ...
    goals = [
      ...
      ('Consistent primary menu destinations',lambda s,r,u:[(node.get('href') if node.name=='a' else node.find_next('a')['href']) for node in s.select('#site-navigation > a, #site-navigation > .nav-menu > .nav-menu-toggle')]==[p[1] for p in config['navigation']]),
      ('Correct menu current-location state',lambda s,r,u:all(u==a['href'] if a.name=='a' and a.get('aria-current')=='page' else (u.startswith(a['href']) if a.name=='a' else a.get('aria-current')=='location') for a in s.select('#site-navigation [aria-current]'))),
